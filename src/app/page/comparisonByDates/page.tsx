        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
          ...
          <div className="flex flex-col">
            ...
            <select
              id="idCadena"
              name="idCadena"
              value={filters.idCadena}
              onChange={handleFilterChange}
              className="border border-gray-300 p-2 rounded text-sm focus:ring-2 focus:ring-blue-500 outline-none"
            >
              <option value="1">Farmacia Kielsa</option>
              <option value="2">Farmacias del Ahorro</option>
              <option value="3">Farmacia Siman</option>
            </select>
          </div>
          ...
          <div className="flex flex-col">
            ...
            >
              ...
              {/* Mapeo dinámico de las categorías traídas de la API */}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left text-gray-600">
            <thead className="text-xs text-gray-700 uppercase bg-gray-100 border-b">
              <tr>
                <th className="px-6 py-4 sticky left-0 bg-gray-100 z-10 border-r shadow-sm">
                  Producto General
                </th>
                ...
                {data?.columnas.map((fecha) => (
                  <th
                    key={fecha}
                    className="px-4 py-4 text-center min-w-[100px] bg-blue-50 text-blue-800 border-r border-blue-100"
                  >
                    {fecha}
                  </th>
                ))}
          ...
          {data?.meta && (
            <div className="flex items-center gap-3">
              <span className="text-xs font-bold text-blue-700 uppercase">
                Fechas ({data.meta.pagina_fechas_actual}/
                {data.meta.paginas_fechas_total})
              </span>
              <div className="flex items-center bg-white border border-blue-200 rounded shadow-sm">
                ...
                <button
                  disabled={
                    data.meta.pagina_fechas_actual >=
                    data.meta.paginas_fechas_total
                  }
                  onClick={() =>
                    setFilters((prev) => ({
