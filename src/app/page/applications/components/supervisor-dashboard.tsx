...
const mockRequests: Request[] = [
  {
    ...
    department: "Planta",
    position: "Operario",
    employeeComments: "Necesito salir para una cita médica importante que no pude programar en otro horario.",
  },
  {
    id: "VAC-15",
    type: "vacation",
...
const generatePDF = (request: Request) => {
  ...
  doc.text("SOLICITUD DE " + (request.type === "vacation" ? "VACACIONES" : "PERMISO"), pageWidth / 2, yPosition, {
    align: "center",
  })
  ...
  const isVacationRequest = (req: Request): req is Request & VacationRequest => {
  ...
  doc.setFontSize(11)
  const statusText =
    request.status === "Pendiente" ? "Pendiente" : request.status === "Aprovada" ? "Aprobado" : "Rechazado"
  ...
  if (request.employeeComments) {
    ...
    doc.setFontSize(11)
    doc.setFont("helvetica", "normal")
    const splitComments = doc.splitTextToSize(request.employeeComments, pageWidth - 2 * margin)
  ...
  if (request.comments) {
    ...
    doc.setFontSize(11)
    doc.setFont("helvetica", "normal")
    const splitComments = doc.splitTextToSize(request.comments, pageWidth - 2 * margin)
    doc.text(splitComments, margin, yPosition)
    yPosition += splitComments.length * 6 + 10
  }

  // Attachments Section
  if (isPermitRequest(request) && request.attachments && request.attachments.length > 0) {
    doc.setFontSize(14)
    doc.setFont("helvetica", "bold")
    ...
    request.attachments.forEach((attachment, index) => {
      doc.text(`${index + 1}. ${attachment.name} (${attachment.type.toUpperCase()})`, margin, yPosition)
      yPosition += 8
    })
    yPosition += 10
  }
  ...
  // Save the PDF
  const fileName = `${request.type === "vacation" ? "Vacaciones" : "Permiso"}_${request.id}_${request.employeeName.replace(/\s+/g, "_")}.pdf`
  doc.save(fileName)
}
...
export default function SupervisorDashboard() {
  ...
  const isPermitRequest = (request: Request): request is Request & PermitRequest => {
    return request.type === "permit"
  }

  const isVacationRequest = (request: Request): request is Request & VacationRequest => {
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Gestión de Solicitudes</h1>
                  ...
                  <TableBody>
                    {paginatedRequests.map((request) => (
                      ...
                      >
                        <TableCell>
                          <div className="flex items-center gap-2 min-w-0">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <div className="text-sm text-gray-500 truncate">{request.department}</div>
                            </div>
                          </div>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell className="whitespace-nowrap text-sm">{request.submittedDate}</TableCell>
                        <TableCell>{getStatusBadge(request.status)}</TableCell>
                        <TableCell>
                          {isVacationRequest(request) ? (
                            <div className="flex items-center gap-1 min-w-0">
                              <Calendar className="h-4 w-4 text-gray-400 flex-shrink-0" />
                              <span className="text-sm truncate">{request.period}</span>
                            </div>
                          ) : (
                            <div className="flex items-center gap-1 min-w-0">
                              <Clock className="h-4 w-4 text-gray-400 flex-shrink-0" />
                              <span className="text-sm truncate">{request.timeRange}</span>
                            </div>
                          )}
                        </TableCell>
                      </TableRow>
              ...
              {paginatedRequests.map((request) => (
                ...
                >
                    ...
                    {isVacationRequest(request) ? (
                      ...
                      <div className="flex items-center gap-2 text-gray-700">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-gray-500">{request.timeRange}</div>
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="mb-3">
                    ...
                    {isPermitRequest(request) && <div className="font-medium text-gray-900 mb-1">{request.reason}</div>}
                    {request.employeeComments && (
                      <div className="text-sm text-blue-600 line-clamp-2">{request.employeeComments}</div>
                    )}
                  </div>
            ...
            <DialogHeader>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <DialogTitle>
                      Detalles de {selectedRequest?.type === "vacation" ? "Vacaciones" : "Permiso"} -{" "}
                      {selectedRequest?.id}
                    </DialogTitle>
                    <DialogDescription>Revise y gestione la solicitud del empleado</DialogDescription>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  {selectedRequest && (
            ...
            {selectedRequest && (
              <div className="space-y-6">
                  ...
                  <CardContent>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="text-lg font-semibold text-blue-600">{selectedRequest.employeeName}</p>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-1">
                          <Calendar className="h-4 w-4 text-gray-400" />
                          <span className="text-sm">{selectedRequest.submittedDate}</span>
                        </div>
                      </div>
                    </div>
                  </CardContent>
                ...
                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                  <div>
                    <Label className="text-sm font-medium text-gray-500">Estado de la Solicitud</Label>
                  </div>
                ...
                <Card>
                  <CardContent className="pt-6">
                    {isVacationRequest(selectedRequest) ? (
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <Label className="text-sm font-medium text-gray-500">Período de Vacaciones</Label>
                          <p className="text-sm">{selectedRequest.period}</p>
                        </div>
                        <div>
                          <Label className="text-sm font-medium text-gray-500">Días Totales</Label>
                          <p className="text-sm">{selectedRequest.days} días</p>
                        </div>
                        <div>
                          <Label className="text-sm font-medium text-gray-500">Días Laborables</Label>
                          <p className="text-sm">{selectedRequest.workDays} días</p>
                        </div>
                        <div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <Label className="text-sm font-medium text-gray-500">Fecha del Permiso</Label>
                          <div className="flex items-center gap-1">
                            <Calendar className="h-4 w-4 text-gray-400" />
                        ...
                        <div>
                          <Label className="text-sm font-medium text-gray-500">Horario del Permiso</Label>
                          <div className="flex items-center gap-1">
                            <Clock className="h-4 w-4 text-gray-400" />
                            <span className="text-sm">{selectedRequest.timeRange}</span>
                          </div>
                        </div>
                        <div>
                          <Label className="text-sm font-medium text-gray-500">Motivo del Permiso</Label>
